...
export default async function ResponseDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <Link href="/quality/responses" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-purple-600">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <Link href={`/quality/responses/${params.id}/edit`} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm bg-muted/30 hover:bg-slate-200 text-foreground/80 rounded-lg font-medium transition-colors">
            <Pencil className="w-3.5 h-3.5" /> Edit
          </Link>
          <span className={`text-xs font-medium px-3 py-1 rounded-full self-start ${STATUS_STYLES[response.status] ?? 'bg-muted/30 text-muted-foreground'}`}>
            {response.status}
          </span>
        </div>
      </div>
