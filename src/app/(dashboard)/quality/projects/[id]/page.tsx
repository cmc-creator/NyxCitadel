...
export default async function QapiProjectDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      {/* Back + header */}
      <div>
        <Link href="/quality/projects" className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-700 mb-4">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="text-sm font-mono text-slate-400">{p.projectNumber}</span>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <DeleteButton apiPath={`/api/qapi/projects/${params.id}`} redirectPath="/quality/projects" label="QAPI project" />
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-4">
            ...
            <dl className="space-y-3 text-sm">
              <div>
                ...
                <dd className="text-slate-800 font-medium mt-0.5">{formatDate(p.startDate)}</dd>
              </div>
              <div>
                <dt className="text-xs text-slate-500 flex items-center gap-1">
                  <Clock className="w-3 h-3" /> Target Date
                </dt>
                <dd className={`font-medium mt-0.5 ${isOverdue ? 'text-orange-600' : 'text-slate-800'}`}>
              ...
              {p.owner && (
                <div>
                  ...
                  <dd className="text-slate-800 mt-0.5">{p.owner}</dd>
                </div>
              )}
              {p.team && (
                <div>
                  <dt className="text-xs text-slate-500 flex items-center gap-1"><Users className="w-3 h-3" /> Team</dt>
                  <dd className="text-slate-700 text-xs mt-0.5 whitespace-pre-wrap">{p.team}</dd>
                </div>
              )}
              {p.relatedMetricKey && (
                <div>
        ...
        <div className="lg:col-span-2 space-y-5">
          <div className="bg-white rounded-xl border border-slate-200 p-5 space-y-4">
            <h2 className="text-sm font-semibold text-slate-700 flex items-center gap-2">
              <FileText className="w-4 h-4 text-slate-400" /> Problem & Aim
            </h2>
            <div>
              <p className="text-xs font-medium text-slate-500 mb-1">Problem Statement</p>
              <p className="text-sm text-slate-700 bg-slate-50 rounded-lg p-3 whitespace-pre-wrap">{p.problemStatement}</p>
            </div>
            <div>
              <p className="text-xs font-medium text-slate-500 mb-1">Project Aim</p>
              <p className="text-sm text-slate-700 bg-slate-50 rounded-lg p-3 whitespace-pre-wrap">{p.aim}</p>
            </div>
          </div>

          {p.interventions && (
            <div className="bg-white rounded-xl border border-slate-200 p-5">
              <h2 className="text-sm font-semibold text-slate-700 mb-2">Interventions</h2>
              <p className="text-sm text-slate-700 whitespace-pre-wrap">{p.interventions}</p>
            </div>
          )}

          {p.outcome && (
            <div className="bg-white rounded-xl border border-slate-200 p-5">
              <h2 className="text-sm font-semibold text-slate-700 mb-2">Outcomes</h2>
              <p className="text-sm text-slate-700 whitespace-pre-wrap">{p.outcome}</p>
            </div>
          )}
