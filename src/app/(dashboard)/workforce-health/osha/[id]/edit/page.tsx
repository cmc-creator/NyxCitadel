...
export default function EditOshaLogPage() {
  ...
  useEffect(() => {
    fetch(`/api/workforce-health/osha/${id}`)
      .then(r => r.json())
      .then(d => {
        setData(d);
        setRecordable(!!d.recordable);
        setPrivacyCase(!!d.privacyCase);
        setLoading(false);
      })
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const res = await fetch(`/api/workforce-health/osha/${id}`, {
      method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload),
    });
    if (res.ok) { router.push(`/workforce-health/osha/${id}`); router.refresh(); }
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div>
        <a href={`/workforce-health/osha/${id}`} className="inline-flex items-center gap-1 text-sm text-slate-500 hover:text-teal-500 mb-3">
