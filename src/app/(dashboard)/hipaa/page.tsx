...
const riskAssessment = {
  lastConducted: '2025-10-15',
  nextDue: '2026-10-15',
  status: 'CURRENT',
  conductor: 'IT Security / Compliance Officer',
  finding: 'Medium risk — 2 gaps identified (mobile device encryption, workforce training)',
};
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Active BAAs', value: 14, icon: FileText, color: 'text-blue-400' },
          { label: 'BAAs Expiring <90d', value: 2, icon: Clock, color: 'text-amber-400' },
          { label: 'Open Breaches', value: 1, icon: AlertTriangle, color: 'text-red-400' },
          { label: 'Risk Assessment', value: 'Current', icon: CheckCircle, color: 'text-emerald-400' },
        ].map(s => (
      ...
      <div className="rounded-xl bg-slate-800/50 border border-white/10 p-5">
        <div className="flex items-center justify-between mb-3">
          <p className="font-semibold text-white text-sm">HIPAA Security Risk Assessment</p>
          <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700">Current</span>
        </div>
        <div className="grid md:grid-cols-3 gap-4 text-xs">
          <div><p className="text-slate-400">Last Conducted</p><p className="text-white font-semibold mt-0.5">{riskAssessment.lastConducted}</p></div>
          <div><p className="text-slate-400">Next Due</p><p className="text-white font-semibold mt-0.5">{riskAssessment.nextDue}</p></div>
          <div><p className="text-slate-400">Conducted By</p><p className="text-white font-semibold mt-0.5">{riskAssessment.conductor}</p></div>
        </div>
        <div className="mt-3 p-3 rounded-lg bg-amber-500/10 border border-amber-500/20">
          <p className="text-xs text-amber-200"><span className="font-semibold text-amber-300">Finding: </span>{riskAssessment.finding}</p>
        </div>
      </div>
      ...
      <div className="rounded-xl bg-slate-800/50 border border-white/10 p-5">
        ...
        <div className="grid md:grid-cols-2 gap-2">
          {[
            ...
            { item: 'BAA in place for all business associates', status: true },
