...
export default function NewPdmpCheckPage() {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div>
        <a href="/pharmacy/pdmp" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-teal-600 mb-3">
        ...
        <p className="text-sm text-muted-foreground mt-0.5">Document a Prescription Drug Monitoring Program query. Patient identifiers are stored as initials only.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-card rounded-xl border border-border divide-y divide-border/30">
        <div className="px-6 py-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-muted-foreground mb-1">Check Date *</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-muted-foreground mb-1">Patient DOB</label>
              <input name="patientDob" type="date" className="form-input w-full" />
            </div>
            <div>
              <label className="block text-xs font-medium text-muted-foreground mb-1">Prescriber ID / Name *</label>
              <input name="prescriberId" type="text" required className="form-input w-full" />
            </div>
          </div>
          <div>
            <label className="block text-xs font-medium text-muted-foreground mb-1">Prescription Type *</label>
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          {significantFinding && (
            <div>
              <label className="block text-xs font-medium text-muted-foreground mb-1">Finding Notes *</label>
        ...
        <div className="px-6 py-4 flex justify-end gap-3">
          <a href="/pharmacy/pdmp" className="px-4 py-2 text-sm rounded-lg border border-border text-muted-foreground hover:bg-muted/20">Cancel</a>
