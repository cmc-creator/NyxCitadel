...
export default function NewHaiSurveillancePage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const data = {
      ...
      submittedToNhsn:(form.elements.namedItem('submittedToNhsn') as HTMLInputElement).checked,
      notes:          (form.elements.namedItem('notes') as HTMLTextAreaElement).value || null,
    };

    const res = await fetch('/api/infection-control/hai', {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data),
    });

    if (res.ok) { router.push('/infection-control/hai'); router.refresh(); }
    else {
      const body = await res.json();
      setError(body.error ?? 'Failed to save HAI data.');
      setSaving(false);
    }
  }
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div>
        <a href="/infection-control/hai" className="inline-flex items-center gap-1 text-sm text-slate-500 hover:text-purple-600 mb-3">
