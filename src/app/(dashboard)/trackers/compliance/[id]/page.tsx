...
const STATUS_OPTIONS = [
  ...
  { value: 'NA', label: 'N/A', color: 'bg-slate-100 text-slate-400' },
];
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <Link href="/trackers/compliance" className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-800 transition">
          <ArrowLeft className="w-4 h-4" /> Back to Compliance Requirements
        </Link>
        <div className="flex items-center gap-2">
          <Link href={`/trackers/compliance/${params.id}/edit`} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg font-medium transition-colors">
            <Pencil className="w-3.5 h-3.5" /> Edit
          </Link>
          <DeleteButton apiPath={`/api/compliance/${params.id}`} redirectPath="/trackers/compliance" label="compliance item" />
          <PrintButton />
        </div>
      </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              ...
              {item.standardRef && <span className="text-xs font-mono text-slate-400">{item.standardRef}</span>}
