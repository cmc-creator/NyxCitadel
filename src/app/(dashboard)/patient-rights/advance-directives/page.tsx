...
export default async function AdvanceDirectivesPage() {
  ...
  const notOffered = records.filter(r => !r.informationOffered).length;
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-3 mb-1">
            ...
            <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-violet-100 text-violet-700">CMS §482.13(b)(3)</span>
          </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          { label: 'Info Offered at Admit',  value: records.filter(r => r.informationOffered).length, color: 'text-blue-400' },
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-white/5">
            {records.length === 0 ? (
              ...
              <tr key={r.id} className={`hover:bg-white/5 transition-colors ${!r.informationOffered ? 'bg-red-500/5' : ''}`}>
                ...
                <td className="px-4 py-3 text-slate-300 text-xs">{r.adType ?? '—'}</td>
                <td className="px-4 py-3">
                  {r.adOnFile ? <CheckCircle className="w-4 h-4 text-emerald-400" /> : <span className="text-xs text-slate-500">—</span>}
                </td>
                <td className="px-4 py-3">
                  {r.informationOffered
                    ? <CheckCircle className="w-4 h-4 text-emerald-400" />
                    : <AlertTriangle className="w-4 h-4 text-red-400" />}
                </td>
                <td className="px-4 py-3">
                  {r.patientDeclined ? <span className="text-xs text-amber-400">Declined</span> : <span className="text-xs text-slate-500">—</span>}
                </td>
              </tr>
            ))}
          </tbody>
...
const mockADs = [
  { id: '1', patientInitials: 'J.D.', admitDate: '2026-03-06', adExists: true, adType: 'Living Will + Healthcare Proxy', adOnFile: true, informationOffered: true, patientDeclined: false },
  { id: '2', patientInitials: 'M.K.', admitDate: '2026-03-04', adExists: true, adType: 'Healthcare Proxy Only', adOnFile: true, informationOffered: true, patientDeclined: false },
  { id: '3', patientInitials: 'R.T.', admitDate: '2026-03-05', adExists: false, adType: null, adOnFile: false, informationOffered: true, patientDeclined: true },
  { id: '4', patientInitials: 'A.B.', admitDate: '2026-03-01', adExists: false, adType: null, adOnFile: false, informationOffered: true, patientDeclined: false },
  { id: '5', patientInitials: 'S.L.', admitDate: '2026-02-18', adExists: false, adType: null, adOnFile: false, informationOffered: false, patientDeclined: false },
];

export default function AdvanceDirectivesPage() {
  const notOffered = mockADs.filter(p => !p.informationOffered).length;
  const onFile = mockADs.filter(p => p.adOnFile).length;
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: 'ADs on File', value: `${onFile}/${mockADs.length}`, color: 'text-emerald-400' },
          { label: 'Info Offered at Admit', value: mockADs.filter(p => p.informationOffered).length, color: 'text-blue-400' },
          { label: 'Info NOT Offered', value: notOffered, color: notOffered > 0 ? 'text-red-400' : 'text-emerald-400' },
        ].map(s => (
          <div key={s.label} className="rounded-xl bg-slate-800/50 border border-white/10 p-4 text-center">
            <p className={`text-3xl font-bold ${s.color}`}>{s.value}</p>
      ...
      <div className="rounded-xl bg-slate-800/50 border border-white/10 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-white/5">
            {mockADs.map(p => (
              <tr key={p.id} className={`hover:bg-white/5 transition-colors ${!p.informationOffered ? 'bg-red-500/5' : ''}`}>
                <td className="px-4 py-3 font-bold text-white">{p.patientInitials}</td>
                <td className="px-4 py-3 text-slate-400 text-xs">{p.admitDate}</td>
                <td className="px-4 py-3">
