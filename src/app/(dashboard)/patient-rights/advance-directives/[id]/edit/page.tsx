...
export default function EditAdvanceDirectivePage() {
  ...
  useEffect(() => {
    fetch(`/api/patient-rights/advance-directives/${id}`)
      .then(r => r.json())
      .then(d => {
        setData(d);
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const res = await fetch(`/api/patient-rights/advance-directives/${id}`, {
    ...
    <div className="max-w-2xl space-y-6">
      ...
      <form key={data.id} onSubmit={handleSubmit} className="bg-white rounded-xl border border-slate-200 divide-y divide-slate-100">
        ...
        <div className="px-6 py-4 flex items-center justify-between gap-3">
          <DeleteButton
            apiPath={`/api/patient-rights/advance-directives/${id}`}
            redirectPath="/patient-rights/advance-directives"
