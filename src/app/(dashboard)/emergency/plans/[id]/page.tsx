...
export default async function EmergencyPlanDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="space-y-6 max-w-3xl">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          {plan.documentUrl && (
            <a href={plan.documentUrl} target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center gap-1 text-sm text-blue-600 border border-blue-200 bg-blue-950/20 px-3 py-1.5 rounded-lg hover:bg-blue-100">
      ...
      {/* Alerts */}
      {isOverdue && (
        <div className="bg-red-950/20 border border-red-200 rounded-xl px-4 py-3 flex items-start gap-2">
      ...
      {isDueSoon && (
        <div className="bg-amber-950/20 border border-amber-200 rounded-xl px-4 py-3 flex items-start gap-2">
      ...
      <div className="bg-card rounded-xl border border-border p-5">
        <h2 className="text-sm font-semibold text-foreground/80 mb-4 flex items-center gap-2">
          <FileText className="w-4 h-4 text-muted-foreground/70" /> Plan Details
        </h2>
      ...
      {plan.summary && (
        <div className="bg-card rounded-xl border border-border p-5">
          <h2 className="text-sm font-semibold text-foreground/80 mb-2">Summary / Scope</h2>
          <p className="text-sm text-foreground/80 whitespace-pre-wrap">{plan.summary}</p>
        </div>
      )}
      ...
      <div className="flex gap-3">
        <Link
          ...
          className="inline-flex items-center gap-1.5 text-sm font-medium text-blue-700 bg-blue-950/20 border border-blue-200 px-3 py-2 rounded-lg hover:bg-blue-100"
        >
          View Drills →
        </Link>
        <Link
