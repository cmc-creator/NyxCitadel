...
export async function GET() {
  ...
  const [
    ...
    prisma.policy.count({ where: { facilityId, deletedAt: null, nextReviewDate: { lt: now }, status: 'ACTIVE' } }),
    ...
    prisma.hipaaBreachLog.count({ where: { facilityId, deletedAt: null, status: { notIn: ['CLOSED', 'REPORTED_TO_HHS'] } } }),
    ...
    prisma.incidentReport.findMany({
      where: { facilityId, deletedAt: null, severity: 'SENTINEL' },
