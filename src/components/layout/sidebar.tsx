...
import { RegulatoryUpdatesBadge } from '@/components/regulatory/RegulatoryUpdatesBadge';
...
const navItems: NavItem[] = [
  ...
  { href: '/archives', label: 'Compliance Archive', icon: Archive },
  {
    ...
    children: [
      ...
      { href: '/regulatory-updates', label: 'Regulatory Updates',   icon: Newspaper },
...
function NavLink({
  ...
  const closeSidebar = useContext(SidebarCloseCtx);
  ...
  return (
    ...
    >
      ...
      {item.href === '/my-day'
        ? <MyDayBadge />
        : item.href === '/regulatory-updates'
        ? <RegulatoryUpdatesBadge />
        : item.badge && (
          <span
            className={cn(
...
function SidebarContent({ onClose }: { onClose?: () => void }) {
  return (
    <SidebarCloseCtx.Provider value={onClose ?? null}>
      <aside className="sidebar w-64 h-full flex flex-col">
        {/* Branding */}
        <div className="flex items-center justify-between px-4 py-4 border-b border-white/8 flex-shrink-0" style={{background: 'linear-gradient(135deg, hsl(228 45% 5%) 0%, hsl(228 42% 7%) 100%)'}}>
...
export function Sidebar({ mobileOpen, onClose }: { mobileOpen?: boolean; onClose?: () => void }) {
  return (
    <>
      ...
      <div className="hidden lg:flex flex-col fixed left-0 top-0 bottom-0 z-30 w-64">
        <SidebarContent />
      </div>
      ...
      {mobileOpen && (
        ...
        >
          ...
          />
          {/* Drawer */}
          <div className="absolute left-0 top-0 bottom-0 w-72 z-50 flex flex-col bg-background shadow-2xl animate-in slide-in-from-left duration-200">
